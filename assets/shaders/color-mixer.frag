...
void main(){
    float red_color = dot(vec4(fs_in.color, 1.0), red);
    float blue_color = dot(vec4(fs_in.color, 1.0), blue);
    float green_color = dot(vec4(fs_in.color, 1.0), green);
    frag_color = vec4(red_color, green_color, blue_color, 1.0);
}