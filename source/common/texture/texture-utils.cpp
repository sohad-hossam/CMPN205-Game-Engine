...
our::Texture2D* our::texture_utils::loadImage(const std::string& filename, bool generate_mipmap) {
    ...
    // Create a texture
    our::Texture2D* texture = new our::Texture2D(); //we genrated a texture
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, size.x, size.y, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels); //used for defining texture images
    glGenerateMipmap(GL_TEXTURE_2D);//1- averaging of pixels
